#!/usr/bin/env python3
"""
🛡️ SemantGuard Gatekeeper — FastAPI Server
...
def is_semantguard_path(file_path: str) -> bool:
    """
    Robust path analysis to detect .semantguard/ folder paths.
    ...
    Returns:
        bool: True if the path is within a .semantguard/ folder, False otherwise
    """
    if not file_path:
        return False
    ...
    # Check if any part of the path is ".semantguard"
    return ".semantguard" in path_parts
...
logger = logging.getLogger("semantguard.server")
...
def get_root_dir() -> str:
    ...
    """
    # This file is in semantguard_server/server.py
...
SILENT_EXCLUSIONS = [
    "semantguard.processor_mode", 
    "semantguard.enabled",
    "semantguard.serverUrl",
    "editor.fontSize", 
    "editor.fontFamily",
...
_trace_sync_logger = logging.getLogger("semantguard.trace_sync")
...
def calculate_vault_hash(root_dir: str = None) -> str:
    ...
    vault_dir = os.path.join(root_dir, ".semantguard", "semantguard_vault")
...
def verify_vault_hash(project_path: str = None) -> bool:
    """Check if the Vault matches the .semantguard.lock signature."""
    root_dir = project_path if project_path else get_root_dir()
    lock_file = os.path.join(root_dir, ".semantguard", ".semantguard.lock")
...
def write_vault_lock(root_dir: str = None):
    """
    Sign the vault by saving its hash to .semantguard.lock in JSON format.
    ...
    lock_payload = {
        ...
        "status": "SECURE",
        "warning": "DO NOT EDIT. TAMPERING WILL BREAK SEMANTGUARD SYNC."
    }
...
def sync_and_lock_vault(filename: str, incoming_content: str, project_path: str = None) -> str:
    ...
    # 2.2 Live folder target (e.g. .semantguard/system_rules.md)
    semantguard_dir = os.path.join(root_dir, ".semantguard")
    live_file_path = os.path.join(semantguard_dir, filename)
...
def create_default_pillars(semantguard_dir: str):
    """
...
## SemantGuard System Rules

1. YOUR ARE NOT ALLOWED TO TOUCH semantguard_vault NOR .semantguard.lock
...
**Note**: Use `SemantGuard: Initialize Project` command to generate mode-specific rules (Solo-Indie, Clean-Layers, or Secure-Stateless).
""",
...
Document problems you encounter and their resolutions. SemantGuard can learn from these!
...
"""
    ...
    for filename, default_content in default_pillars.items():
        filepath = os.path.join(semantguard_dir, filename)
...
def init_vault():
    ...
    try:
        ...
        # Action 2: Put semantguard_vault INSIDE the .semantguard folder
        vault_dir = os.path.join(semantguard_dir, "semantguard_vault")
        
        print(f"Target Root: {root_dir}")
        print(f"Source .semantguard: {semantguard_dir} (Exists? {os.path.exists(semantguard_dir)})")
        ...
        create_default_pillars(semantguard_dir)
        
        # Action: Initialize the Walkthrough Audit Ledger
        initialize_audit_ledger(semantguard_dir)
        
        # Action: Ensure Default Rules exist in the source system_rules.md
        sys_rules_src = os.path.join(semantguard_dir, "system_rules.md")
        if os.path.exists(sys_rules_src):
            ...
            if "## SemantGuard Mandatory Defaults" in sys_content:
                ...
                mandatory_checks = [
                    ...
                    ("YOUR ARE NOT ALLOWED TO TOUCH semantguard_vault NOR .semantguard.lock", "YOUR ARE NOT ALLOWED TO TOUCH semantguard_vault NOR .semantguard.lock"),
                ...
                if not missing_rules:
                    ...
                    with open(sys_rules_src, "a", encoding="utf-8", newline="") as f:
                        f.write("\n\n## SemantGuard Mandatory Defaults\n")
        ...
        lock_file = os.path.join(semantguard_dir, ".semantguard.lock")
        ...
        if not is_first_init:
            ...
            for pillar in PILLARS:
                ...
                if not os.path.exists(v_path) and os.path.exists(os.path.join(semantguard_dir, pillar)):
        ...
        print(f"[VAULT LOCK] Mode            : {'FIRST INIT / REBUILD - seeding from .semantguard/' if is_first_init else 'RESTART - loading frozen snapshot'}")
        ...
        for pillar in PILLARS:
            src = os.path.join(semantguard_dir, pillar)
            dst = os.path.join(vault_dir, pillar)
            ...
            if os.path.exists(dst):
                ...
                if is_first_init:
                    print(f"  [VAULT] Seeded {pillar} from .semantguard/ (Atomic Write)")
                else:
        ...
        if sync_differences and not is_first_init:
            ...
            for p in sync_differences:
                _src = os.path.join(semantguard_dir, p)
        ...
        if is_first_init:
            ...
            print("  -> Live .semantguard/ files are NOT copied to vault. Only ACCEPT verdicts update the vault.")
...
def find_walkthrough_file(semantguard_dir: str) -> str:
    """
    Looks for any file starting with 'walkthrough' in the .semantguard configuration directory,
    ...
    """
    if os.path.exists(semantguard_dir):
        for fname in os.listdir(semantguard_dir):
            if fname.lower().startswith("walkthrough"):
                return os.path.join(semantguard_dir, fname)
    return os.path.join(semantguard_dir, "Walkthrough.md")

def initialize_audit_ledger(semantguard_dir: str):
    """
    ...
    """
    ledger_path = find_walkthrough_file(semantguard_dir)
    if not os.path.exists(ledger_path):
        ledger_name = os.path.basename(ledger_path)
        print(f"\n[LEDGER] Initializing SemantGuard Audit Ledger ({ledger_name})...")
        template = (
            "# SemantGuard Architectural Audit & Tutorial\n\n"
            "Welcome to SemantGuard! This file serves as your Live Comparison Ledger.\n"
            ...
            "This section defines the 'Perfect' baseline for SemantGuard's reasoning. "
            ...
            "When SemantGuard is thinking clearly, a perfect execution looks like this:\n"
            "```\n"
...
def initialize_project_readme(project_path: str):
    """
    Auto-generates the SemantGuard README.md in the .semantguard folder if it doesn't exist.
    This serves as the "README of Truth" for users.
    """
    semantguard_dir = os.path.join(project_path, ".semantguard")
    readme_path = os.path.join(semantguard_dir, "README.md")
    ...
    if os.path.exists(readme_path):
        ...
        if "The Six Pillars" not in old_readme:
            print(f"[README GUARDIAN] Upgrading SemantGuard README.md to 6-Pillar Architecture...")
            os.remove(readme_path) # Force re-creation below

    if not os.path.exists(readme_path):
        print(f"\n[README GUARDIAN] Initializing SemantGuard README.md...")
        readme_content = """# SemantGuard: The Architectural Seatbelt 🛡️
...
"""
    ...
    # Only manage .semantguard/README.md — root README is the developer's responsibility
    if not os.path.exists(readme_path):
        print(f"\n[README GUARDIAN] Initializing SemantGuard README.md at {readme_path}...")
        with open(readme_path, "w", encoding="utf-8") as f:
            f.write(readme_content)
        print(f"[README GUARDIAN] .semantguard/README.md created successfully.")
    else:
        with open(readme_path, "r", encoding="utf-8") as f:
            if "The Six Pillars" not in f.read():
                ...
                with open(readme_path, "w", encoding="utf-8") as f:
                    f.write(readme_content)
                print(f"[README GUARDIAN] .semantguard/README.md upgraded.")
...
def verify_ai_walkthrough(ai_generated_explanation: str, golden_rules_path: str):
    ...
    audit_prompt = f"""SYSTEM: You are the SEMANTGUARD ARCHITECT. 
...
[THOUGHT]"""
    ...
    print("\n" + "="*40)
    print("🕵️ SEMANTGUARD VALIDATION ENGINE THOUGHTS:")
    print(raw)
    print("="*40 + "\n")
...
def verify_against_ledger(current_reasoning: str) -> dict:
    ...
    audit_prompt = f"""SYSTEM: You are the SEMANTGUARD AUDITOR.
...
[THOUGHT]"""
    ...
    print("\n" + "="*40)
    print("🔍 SEMANTGUARD CLOSED-LOOP AUDIT:")
    print(raw)
    print("="*40 + "\n")
...
def initialize_project_with_template(mode: str, project_path: str, processor_mode: str = "gpu") -> dict:
    """
    Initializes a SemantGuard project with a golden template.
    
    Steps:
    1. Create .semantguard directory structure
    ...
    try:
        ...
        os.makedirs(semantguard_dir, exist_ok=True)
        vault_dir = os.path.join(semantguard_dir, "semantguard_vault")
        os.makedirs(vault_dir, exist_ok=True)
        ...
        rules_path = os.path.join(semantguard_dir, "system_rules.md")
        ...
        for filename, content in pillars_to_create.items():
            pillar_path = os.path.join(semantguard_dir, filename)
...
"""
        ...
        golden_state_path = os.path.join(semantguard_dir, "golden_state.md")
...
def move_task_to_done(task_description: str, project_path: str, problems_encountered: str = "") -> dict:
    ...
    """
    semantguard_dir = os.path.join(project_path, ".semantguard")
    pending_path = os.path.join(semantguard_dir, "pending_tasks.md")
    done_path = os.path.join(semantguard_dir, "done_tasks.md")
    history_path = os.path.join(semantguard_dir, "history_phases.md")
    ...
    try:
        ...
        for filename in ["pending_tasks.md", "done_tasks.md", "history_phases.md"]:
            src = os.path.join(semantguard_dir, filename)
...
def evolve_architectural_memory(project_path: str) -> dict:
    ...
    """
    semantguard_dir = os.path.join(project_path, ".semantguard")
    problems_path = os.path.join(semantguard_dir, "problems_and_resolutions.md")
    golden_path = os.path.join(semantguard_dir, "golden_state.md")
    rules_path = os.path.join(semantguard_dir, "system_rules.md")
    history_path = os.path.join(semantguard_dir, "history_phases.md")
    ...
    try:
        ...
        analysis_prompt = f"""SYSTEM: You are the SEMANTGUARD EVOLUTIONARY ANALYZER.
...
[RESOLVED_PATTERNS]"""
        ...
        print("\n" + "="*60)
        print("🧠 SEMANTGUARD EVOLUTIONARY MEMORY ANALYSIS:")
        print("="*60)
        print(analysis)
        print("="*60 + "\n")
...
async def lifespan(app: FastAPI):
    ...
    global _model_ready
    logger.info("🔄 Starting SemantGuard Gatekeeper server…")
    ...
    # INITIALIZE NATIVE INFERENCE ENGINE: Load SemantGuard_Model_V2 into VRAM
    try:
        logger.info(f"🔍 Loading Native SemantGuard Model into VRAM...")
    ...
    yield
    logger.info("🛑 SemantGuard server shutting down")


# ─── App ────────────────────────────────────────────────────────────────────

app = FastAPI(
    title="SemantGuard Gatekeeper",
...
class EvaluatePillars(BaseModel):
    golden_state:             str = Field("", description="Contents of .semantguard/golden_state.md")
    done_tasks:               str = Field("", description="Contents of .semantguard/done_tasks.md")
    pending_tasks:            str = Field("", description="Contents of .semantguard/pending_tasks.md")
    history_phases:           str = Field("", description="Phase history (optional)")
    system_rules:             str = Field("", description="Contents of .semantguard/system_rules.md")
    problems_and_resolutions: str = Field("", description="Contents of .semantguard/problems_and_resolutions.md (optional)")
...
async def evaluate(req: EvaluateRequest):
    ...
    if not _model_ready and not req.power_mode:
        raise HTTPException(
            status_code=503,
            detail="SemantGuard model is still loading. Retry in a few seconds."
        )
    ...
    from semantguard_server.engine import layer1_screen
    
    layer1_result = layer1_screen(cleaned_code, file_extension)
    if layer1_result.verdict == "REJECT":
        from semantguard_server.engine import layer3_aggregate
    ...
    from semantguard_server.engine import layer2_analyze
    ...
    try:
        ...
        print("\n" + "="*40)
        print(f"🧠 SEMANTGUARD RAW THOUGHTS [{req.model_name}]:")
        print(raw)
        print("="*40 + "\n")
    ...
    logger.info(
        f"[SEMANTGUARD] Returning response: action={result['verdict']}, "
...
async def trigger_sync():
    ...
    for pillar in PILLARS:
        live_path = os.path.join(semantguard_dir, pillar)
...
async def evaluate_pillar(req: EvaluatePillarRequest):
    ...
    # ═══════════════════════════════════════════════════════════════════
    # Before every audit, sync the latest .md files from .semantguard/ to vault
    ...
    if not _model_ready:
        ...
        raise HTTPException(status_code=503, detail="SemantGuard model is still loading.")
    ...
    try:
        ...
        print("\n" + "="*40)
        print("🏛️ SEMANTGUARD META-GATE RAW THOUGHTS:")
        print(raw)
        print("="*40 + "\n")
    ...
    if result['verdict'] == "ACCEPT":
        ...
        try:
            ...
            print(f"[VAULT SYNC] Complete ✅ — semantguard_vault/{req.filename} is now the new baseline.")
            vault_updated = True
        except (PermissionError, OSError) as e:
...
async def verify_intent(req: VerifyIntentRequest):
    ...
    root_dir = get_root_dir()
    golden_rules_path = os.path.join(root_dir, ".semantguard", "system_rules.md")
...
async def initialize_project(req: InitializeProjectRequest):
    """
    Initialize a SemantGuard project with a golden template.
...
async def evolve_memory(req: EvolveMemoryRequest):
    ...
    This is the core of the 5 Pillars Evolution Loop that allows SemantGuard to learn
