#!/usr/bin/env python3
"""
Convert SemantGuard model from .safetensors to .bin format
...
def convert_safetensors_to_bin(model_path: str):
    ...
    print(f"📦 Converting SemantGuard model format...")
...
def main():
    ...
    default_model_path = base_dir / "SemantGuard_Model_V2"
    
    parser = argparse.ArgumentParser(description="Prepare SemantGuard model with comprehensive diagnostics")
    ...
    print("=" * 80)
    print("🛡️  SEMANTGUARD MODEL PREPARATION & DIAGNOSTICS")
    print("=" * 80)
        ...
        if model_path.parent.exists():
            try:
                similar_dirs = [d for d in model_path.parent.iterdir() 
                              if d.is_dir() and 'semantguard' in d.name.lower()]
                if similar_dirs:
    ...
    print("=" * 80)
    if success:
        print("✅ SEMANTGUARD MODEL PREPARATION COMPLETED SUCCESSFULLY")
        print("")
        print("🚀 Ready to start SemantGuard server!")
        print("   Next steps:")
        ...
        print("❌ SEMANTGUARD MODEL PREPARATION FAILED")
        print("")
