#!/usr/bin/env python3
"""
🛡️ SemanticGuard Gatekeeper — HuggingFace Model Uploader/Downloader
Handles pushing SemanticGuard_Model_V2 to HF Hub and downloading on new machines.

Usage:
    # Upload (one-time, for the model author)
    python hf_model_manager.py upload --repo YOUR_USERNAME/SemanticGuard_Model_V2

    # Download (for new users)
    python hf_model_manager.py download --repo YOUR_USERNAME/SemanticGuard_Model_V2

    # Check if model exists locally or on HF
    python hf_model_manager.py status --repo YOUR_USERNAME/SemanticGuard_Model_V2
"""
...
ADAPTER_DIR = Path(__file__).parent.parent / "SemanticGuard_Model_V2"
...
def cmd_upload(repo: str, token: str | None):
    ...
    print(f"🚀 Uploading SemanticGuard_Model_V2 → {repo}")
...
def cmd_download(repo: str, token: str | None):
    ...
    if ADAPTER_DIR.exists() and (ADAPTER_DIR / "adapter_model.safetensors").exists():
        ...
        print("   (Delete SemanticGuard_Model_V2/ and re-run to force re-download)")
        return
    ...
    print(f"\n✅ SemanticGuard_Model_V2 downloaded to {ADAPTER_DIR.resolve()}")
...
def main():
    parser = argparse.ArgumentParser(description="🛡️ SemanticGuard Model Manager (HuggingFace)")
    parser.add_argument("command", choices=["upload", "download", "status"])
    parser.add_argument("--repo",  required=True, help="HF repo ID, e.g. your-username/SemanticGuard_Model_V2")
