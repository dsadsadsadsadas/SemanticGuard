#!/usr/bin/env python3
"""
Convert SemanticGuard model from .safetensors to .bin format
...
def convert_safetensors_to_bin(model_path: str):
    ...
    print(f"📦 Converting SemanticGuard model format...")
...
def main():
    ...
    default_model_path = base_dir / "SemanticGuard_Model_V2"
    
    parser = argparse.ArgumentParser(description="Prepare SemanticGuard model with comprehensive diagnostics")
    ...
    print("=" * 80)
    print("🛡️  SEMANTICGUARD MODEL PREPARATION & DIAGNOSTICS")
    print("=" * 80)
        ...
        if model_path.parent.exists():
            try:
                similar_dirs = [d for d in model_path.parent.iterdir() 
                              if d.is_dir() and 'semanticguard' in d.name.lower()]
                if similar_dirs:
    ...
    print("=" * 80)
    if success:
        print("✅ SEMANTICGUARD MODEL PREPARATION COMPLETED SUCCESSFULLY")
        print("")
        print("🚀 Ready to start SemanticGuard server!")
        print("   Next steps:")
        ...
        print("❌ SEMANTICGUARD MODEL PREPARATION FAILED")
        print("")
