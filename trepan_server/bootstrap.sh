#!/usr/bin/env bash
# 🛡️ Trepan Gatekeeper — Linux/macOS Bootstrap Script

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(dirname "$SCRIPT_DIR")"
VENV_DIR="$PROJECT_ROOT/venv_trepan_server"
SERVER_PORT=8000
ADAPTER_DIR="$PROJECT_ROOT/Trepan_Model_V2"
...
if [ ! -f "$ADAPTER_DIR/adapter_model.safetensors" ]; then
    ...
    echo "        Ensure the Trepan_Model_V2 folder is in the project root."
    exit 1
fi
...
# ── 8. Start server ───────────────────────────────────────
echo ""
echo "[INFO] Starting Trepan Gatekeeper on port $SERVER_PORT..."
echo "[INFO] Press Ctrl+C to stop."
echo ""
cd "$PROJECT_ROOT"
python3 -m uvicorn trepan_server.server:app \
    --host 127.0.0.1 \
    --port "$SERVER_PORT" \
    --log-level info
